...
class SARSA(BaseAlgorithm):
    ...
    Algorithme on-policy qui apprend une politique en utilisant
    l'action suivante de la politique courante.
    """
    ...
    def __init__(self, 
        ...
        """
        super().__init__("SARSA", state_space_size, action_space_size)
    ...
    def from_config(cls, config: Dict[str, Any], environment):
        """Crée SARSA depuis une configuration."""
        return cls(
    ...
    def train(self, environment, num_episodes: int, verbose: bool = False):
        """
        Entraîne SARSA sur l'environnement.
        ...
        """
        if verbose:
            print(f"Entraînement SARSA sur {num_episodes} épisodes...")
        ...
        for episode in range(num_episodes):
            ...
            # Sélectionner la première action parmi les actions valides
            ...
            while not done:
                ...
                if not done:
                    # Sélectionner la prochaine action selon la politique courante
                    next_valid_actions = environment.valid_actions
                    if np.random.random() < self.epsilon:
                        next_action = np.random.choice(next_valid_actions)
                    else:
                        next_q_values = [self.q_function[next_state, a] for a in next_valid_actions]
                        next_best_action_idx = np.argmax(next_q_values)
                        next_action = next_valid_actions[next_best_action_idx]
                    
                    # Mettre à jour Q(s,a) avec SARSA
                    current_q = self.q_function[state, action]
                    next_q = self.q_function[next_state, next_action]
                    
                    # Formule SARSA: Q(s,a) = Q(s,a) + α[r + γQ(s',a') - Q(s,a)]
                    ...
                    # Épisode terminé, mise à jour finale
                    current_q = self.q_function[state, action]
                    td_target = reward
                    td_error = td_target - current_q
                    self.q_function[state, action] = current_q + self.learning_rate * td_error
